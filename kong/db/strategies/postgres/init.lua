...
local function execute(strategy, statement_name, attributes, options)
  ...
  for i = 1, argc do
    local name  = argn[i]
    local value
    if ttl and name == TTL then
      if not update and attributes[CREATED_AT] then
        value = ttl + attributes[CREATED_AT]
      else
        value = ttl + time()
      end

    elseif i == argc and update and attributes[UNIQUE] then
...
function _M.new(connector, schema, errors)
  ...
  if ttl then
    ...
    ttl_index = escape_identifier(connector, table_name .. "_" .. TTL .. "_idx")
    update_fields_count = update_fields_count + 1
